...
def spiral_data(points, classes):
    ...
    y = np.zeros(points*classes, dtype='uint8')#the fking class
    for class_number in range(classes):
        ix = range(points*class_number, points*(class_number+1))#fking values
        r = np.linspace(0.0, 1, points)  #radius
        t = np.linspace(class_number*4, (class_number+1)*4,num=points) + np.random.randn(points)*0.2#thefking angle
...
class softmaxx:
    def softmax(self,ip):
        ...
        return pbty
#now loss fn i guess bitch
#categorical cross entropy - sum (log(bar_y))
